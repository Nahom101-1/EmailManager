...
export default async function EmailsMapPage() {
        ...
        <div className="page-head">
          <div>
            ...
            <p className="page-sub">
              Connect 2–4 inboxes to see which services live on which address — and what shows up on
              more than one.
            </p>
          </div>
        </div>
  ...
  return (
    <div className="page fade-in">
      ...
      >
        {summary.inboxes.map(
          ({ provider, tag, accountCount, subscriptionCount, activeSubscriptionCount }) => (
            <Card key={provider.id} className="card-pad" style={{ display: "grid", gap: 12 }}>
              ...
              <div className="btn-row">
                <Link
                  href={`/accounts?inbox=${encodeURIComponent(provider.email)}`}
      ...
      <section style={{ marginBottom: 28 }}>
        ...
        summary.sharedSubscriptionGroups.length === 0 ? (
          <Card className="card-pad muted" style={{ fontSize: 13 }}>
            No services detected on more than one inbox yet. After syncing multiple emails,
            duplicates like IMAX show up here.
          </Card>
        ) : (
                ...
                <div className="between" style={{ alignItems: "flex-start", gap: 12 }}>
                  <div className="center gap10">
                    ...
                    <div>
                      ...
                      <div className="row-sub">Account · {g.inboxes.length} inboxes</div>
                    </div>
                  </div>
              ...
              <Card key={`s-${g.key}`} className="card-pad">
                <div className="between" style={{ alignItems: "flex-start", gap: 12 }}>
                  <div className="center gap10">
                    ...
                    <div>
                      <div className="row-title">{g.company}</div>
                      <div className="row-sub">Subscription · {g.inboxes.length} inboxes</div>
                    </div>
                  </div>
                  <span className="chip" style={{ height: 22, fontSize: 11 }}>
                    <Icon name="layers" size={11} />
                ...
                <div className="center gap6 wrap mt10">
                  {g.instances.map((inst) => (
                    ...
                    >
                      ...
                      {inst.amount != null ? ` · $${inst.amount.toFixed(2)}` : ""}
                    </Link>
                  ))}
                </div>
              </Card>
