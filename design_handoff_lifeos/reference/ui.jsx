/* LifeOS UI primitives + icons — exports to window */

const ICONS = {
  dashboard: '<path d="M3 3h7v7H3zM14 3h7v5h-7zM14 12h7v9h-7zM3 14h7v7H3z"/>',
  connect: '<path d="M9 7H6a4 4 0 0 0 0 8h3M15 17h3a4 4 0 0 0 0-8h-3M8 12h8"/>',
  sync: '<path d="M21 2v6h-6M3 12a9 9 0 0 1 15-6.7L21 8M3 22v-6h6M21 12a9 9 0 0 1-15 6.7L3 16"/>',
  subs: '<path d="M2 8h20M6 4h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2zM6 14h4"/>',
  accounts: '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM22 21v-2a4 4 0 0 0-3-3.87M16 3.13A4 4 0 0 1 16 11"/>',
  ...
  eyeOff: '<path d="M9.88 9.88a3 3 0 0 0 4.24 4.24M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61M2 2l20 20"/>',
  db: '<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5M3 12c0 1.66 4 3 9 3s9-1.34 9-3"/>',
  bolt: '<path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/>',
  pause: '<path d="M14 4h-2v16h2zM10 4H8v16h2z"/>',
  dots: '<circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/>',
  filter: '<path d="M22 3H2l8 9.46V19l4 2v-8.54L22 3z"/>',
  bell: '<path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9M10.3 21a1.94 1.94 0 0 0 3.4 0"/>',
  tag: '<path d="M12 2H2v10l9.29 9.29a1 1 0 0 0 1.42 0l7.58-7.58a1 1 0 0 0 0-1.42zM7 7h.01"/>',
  ...
  refresh: '<path d="M3 2v6h6M21 12A9 9 0 0 0 6 5.3L3 8M21 22v-6h-6M3 12a9 9 0 0 0 15 6.7l3-2.7"/>',
  user: '<circle cx="12" cy="8" r="4"/><path d="M4 21v-1a7 7 0 0 1 14 0v1"/>',
  receipt: '<path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1zM8 7h8M8 11h8M8 15h5"/>',
  ...
  note: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M9 13h6M9 17h4"/>',
...
function Provider({ provider, size = "" }) {
  const map = {
    google: { txt: "G", bg: "linear-gradient(135deg,#4285F4,#34A853 60%,#EA4335)", clip: true },
    imap: { txt: "@", bg: "var(--surface-inset)" },
    outlook: { txt: "O", bg: "#0a64bf" },
  };
  const p = map[provider] || map.imap;
  if (provider === "google") {
    return (
      <span className={"mono-tile " + size} style={{ background: "var(--surface-inset)", color: "#4285F4", fontWeight: 800 }}>G</span>
    );
  }
  if (provider === "outlook") {
    return <span className={"mono-tile " + size} style={{ background: "#0a64bf", color: "#fff", border: 0 }}>O</span>;
  }
  return <span className={"mono-tile " + size} style={{ fontWeight: 600 }}><Icon name="mail" size={16} /></span>;
}

function Tile({ mono, color, size = "" }) {
  return (
    <span className={"mono-tile " + size} style={{ background: (color || "var(--surface-inset)") + "1a", color: color || "var(--ink)", borderColor: (color || "var(--border)") + "33" }}>
      {mono}
    </span>
  );
}
...
function StatusBadge({ status, pulse }) {
  ...
  return (
    <span className={"badge " + s.cls}>
      <span className={"dot" + (pulse ? " pulse" : "")}></span>{s.label}
    </span>
  );
}
...
function Btn({ variant = "", size = "", icon, iconR, children, className = "", ...rest }) {
  ...
  return (
    <button className={cls} {...rest}>
      {icon && <Icon name={icon} size={size === "xs" ? 13 : 15} />}
      {children}
      {iconR && <Icon name={iconR} size={size === "xs" ? 13 : 15} />}
    </button>
  );
}
...
function Conf({ value }) {
  ...
  return (
    <span className={"conf" + (value < 70 ? " low" : "")} title={value + "% confidence"}>
...
Object.assign(window, { Icon, ICONS, Provider, Tile, StatusBadge, STATUS, Btn, Seg, Conf, Card, Field, fmt });
